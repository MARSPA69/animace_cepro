...
let playbackSpeed = 1; // 0 = pause, 1 = normal, 2 = fast, -2 = rewind

// --- Incident storage & smoothing counters ---
let incidents     = [];
let inIncident    = false;
let prevInRed     = false;
let redCounter    = 0;
let greenCounter  = 0;
const TRANSITION_THRESHOLD = 3; // require 3 consecutive frames inside zone

// --- 1) Ovládací panel ---
const ctrlPanel = document.createElement('div');
Object.assign(ctrlPanel.style, {
  position: 'absolute', top: '10px', left: '10px',
  zIndex: 1001, background: 'rgba(255,255,255,0.8)', padding: '4px', borderRadius: '4px'
});
ctrlPanel.innerHTML = `
...
const timer = setInterval(() => {
  ...
  for (let step = 0; step < Math.abs(playbackSpeed); step++) {
    ...
    // d) Přechodové counters
    if (inRed)      { redCounter++;   greenCounter = 0; }
    else            { greenCounter++; redCounter   = 0; }

    // e) Incident logika se zpožděním
    if (inRed && !inIncident && redCounter >= TRANSITION_THRESHOLD) {
      inIncident = true;
      incidents.push({
        inDate:   new Date(),
    ...
    if (!inRed && inIncident && greenCounter >= TRANSITION_THRESHOLD) {
      inIncident = false;
