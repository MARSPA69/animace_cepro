...
// Přidání polygonů na mapu (v části inicializace mapy)
L.geoJSON(smallPoly, {
  color: '#28a745', weight: 3, fillColor: '#90ee90', fillOpacity: 0.3
}).bindPopup('Povolená zóna (green)').addTo(map);

L.geoJSON(segA_poly, {
  color: '#28a745', weight: 2, fillOpacity: 0.2
}).bindPopup('SEG_A poly').addTo(map);

L.geoJSON(segB_poly, {
  color: '#28a745', weight: 2, fillOpacity: 0.2
}).bindPopup('SEG_B poly').addTo(map);

L.geoJSON(segB_mez_poly, {
  color: '#28a745', weight: 2, fillOpacity: 0.2
}).bindPopup('SEG_B_mez poly').addTo(map);

L.geoJSON(segC_poly, {
  color: '#28a745', weight: 2, fillOpacity: 0.2
}).bindPopup('SEG_C poly').addTo(map);

L.geoJSON(bigPoly, {
  color: '#dc3545', weight: 3, dashArray: '5, 10', fillOpacity: 0
...
// --- 4) Nová demo časová osa s přechody mezi zónami ---
const demoTimeline = [];
let t = 0;
...
for(let cycle = 0; cycle < 7; cycle++) {
  ...
  demoTimeline.push({start: t, end: t+5, motion: {name: 'běh', speed: 5.0}, zone: 'toRed'});
  ...
  demoTimeline.push({start: t+20, end: t+25, motion: {name: 'rychlá chůze', speed: 1.4}, zone: 'toGreen'});
  t += 25;
  
  // 3) Přechod do Green (5s) + Green fáze (20s)
  demoTimeline.push({start: t, end: t+5, motion: {name: 'běh', speed: 5.0}, zone: 'toGreen'});
  ...
  demoTimeline.push({start: t+20, end: t+25, motion: {name: 'stání', speed: 0}, zone: 'toRed'});
  t += 25;
}

// --- 5) Vylepšená smyčka animace s přechody mezi zónami ---
let currentPt = turf.point(greenCenter);
let elapsed = 0;
const TIME_STEP = 100; // 100ms = 0.1s pro plynulejší pohyb

// Nastavení cílových bodů pro pohyb
const targets = {
  green: turf.point(greenCenter),
  red: turf.point(redCenter),
  toRed: turf.point(redCenter),
  toGreen: turf.point(greenCenter)
};
...
const timer = setInterval(() => {
  ...
  for (let i = 0; i < speedFactor; i++) {
    ...
    // Detekce incidentů (pouze když vstoupí do Red zóny)
    if (inRed && !prevInRed) {
    ...
    if (inGreen) {
      status = "Pohyb v povolené zóně";
      statusColor = "#28a745";
    } else if (inRed) {
      status = "INCIDENT v zakázané zóně";
      statusColor = "#dc3545";
    } else {
      status = "Pohyb mezi zónami";
      statusColor = "#6c757d";
    }
    
    marker.setPopupContent(`
      <div style="font-size:12px; min-width:220px">
        <b style="color:${statusColor}">${status}</b><hr style="margin:5px 0">
        ...
        <b>Vzdálenost k zóně:</b> ${currentDist} m<br>
        <b>Aktuální zóna:</b> ${zone}
      </div>
    `).openPopup();
  }
