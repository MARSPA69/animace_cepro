...
function stopBoth() {
  ...
  // GNSS/F_GPS markery pryč
  if (window.markerF) { if (typeof window.markerF.closePopup === 'function') window.markerF.closePopup(); map.removeLayer(window.markerF); window.markerF = null; }
  if (window.markerB) { if (typeof window.markerB.closePopup === 'function') window.markerB.closePopup(); map.removeLayer(window.markerB); window.markerB = null; }
...
function applyChannel() {
  ...
  // overlay MESH vždy podle režimu
  const meshOn = (mode === 'mesh' || mode === 'offlinegnss' || mode === 'both');
  toggleMesh(meshOn);

  // ─── BOTH (porovnání) ─────────────────────────────────────────────
  if (mode === 'both') {
    // a) ukliď single, ať nevzniká 3. kulička
    stopSingle();
    bothResetState();

    // b) připrav GNSS benchmark (JEN teď, před nasazením kuliček)
  if (!benchData.length) {
  if (Array.isArray(gnssMaster) && gnssMaster.length) {
    ...
    // c) připrav F_GPS (fusedData), pokud ještě není
    if (!fusedData.length) {
      let fused = null;

      // 1) pokud existuje předpočítaný dataset
      if (window.F_GPS_DATASET?.items?.length) {
        fused = window.F_GPS_DATASET.items.map(r => ({
          lat: r.F_GPS?.lat, lng: r.F_GPS?.lng,
          time: Date.parse(`1970-01-01T${r.TIMESTAMP}Z`),
          speed_mps: r.SPEED_MPS ?? null,
          dist_to_m: r.DIST_TO_M ?? null,
          mesh_id: r.MESH_ID ?? null,
      ...
      // 2) jinak dopočítej z FUSED_GPS.js
      if (!fused || !fused.length) {
        if (window.FUSED_GPS?.buildFusedSeries) {
          const raw = window.FUSED_GPS.buildFusedSeries();
          if (Array.isArray(raw) && raw.length) {
            fused = raw.map(r => ({
              lat: r.lat, lng: r.lng,
              time: (typeof r.time === 'number') ? r.time : Date.parse(`1970-01-01T${r.timestamp}Z`),
              speed_mps: r.speed_mps ?? null,
              dist_to_m: r.dist_to_m ?? null,
              mesh_id: r.mesh_id ?? null,
              matched_count: r.matched_count ?? 0,
      ...
      fusedData = Array.isArray(fused) ? fused : [];
      if (!fusedData.length) {
        console.warn('BOTH: fusedData je prázdné – chybí vstupní datasety pro F_GPS.');
      }
    }

    // ── DEBUG: zde vlož kontrolní log (viz níže) ──
    ...
    // d teprve teď polož kuličky na start
    bothSetStartPositions();
    return;
  }
...
document.getElementById('pauseBtn')?.addEventListener('click', () => {
  ...
  if (mode === 'both') { bothActive = false; if (bothTimer){clearTimeout(bothTimer); bothTimer=null;} }
  else { animationActive = false; if (window.timer){clearTimeout(window.timer); window.timer=null;} }
...
function ensureBothMarkers() {
  ...
  if (window.markerF && typeof window.markerF.getPopup === 'function' && !window.markerF.getPopup()) {
    window.markerF.bindPopup('', { closeButton:false, autoClose:false, closeOnClick:false, offset:[0,-10] }).openPopup();
  } else if (window.markerF) {
    window.markerF.openPopup?.();
  }

  if (window.markerB && typeof window.markerB.getPopup === 'function' && !window.markerB.getPopup()) {
    window.markerB.bindPopup('', { closeButton:false, autoClose:false, closeOnClick:false, offset:[0,-10] }).openPopup();
  } else if (window.markerB) {
    window.markerB.openPopup?.();
  }
}
...
// Zarovná pouze ČASY F_GPS na den GNSS. Pozice se NEMĚNÍ.
function alignFusedToBenchDate() {
  ...
  // vezmeme prvních K F_GPS bodů a k nim najdeme nejbližší GNSS čas
  const K = Math.min(10, fusedData.length);
  const diffs = [];
  for (let i = 0; i < K; i++) {
    const f = fusedData[i];
    const b = nearestByTime(benchData, f.time, 3600); // hodina tolerance
    if (b) diffs.push(b.time - f.time);
  }
  if (!diffs.length) return;

  // robustně – median, ne průměr
  diffs.sort((a,b)=>a-b);
  ...
  console.log('alignFusedToBenchDate(): offset(ms)=', offset, 'samples=', diffs.length);
}

function bothRun(){
  // --- GNSS = VÝHRADNĚ RENDERERDATA (gnssMaster) ---
  if (!Array.isArray(gnssMaster) || !gnssMaster.length) {
    console.warn('BOTH: GNSS master není načten (vyber den).');
    return;
  }
  // čerstvá kopie; žádné aliasy + seřadit dle času
  ...
  if (!fusedData.length) { console.warn('BOTH: chybí fusedData'); return; }

  // --- zarovnání času F_GPS na den/čas GNSS (GNSS se NEMĚNÍ) ---
  alignFusedToBenchDate();  // použij tu robustní median verzi, co jsme si dali dřív

  // --- stopni single, připrav start ---
  stopSingle();
  bothActive = true;
  if (window.timer) { clearTimeout(window.timer); window.timer = null; }
  if (window.marker){ map.removeLayer(window.marker); window.marker = null; }
  ...
  console.log('DBG bothRun sources:', {
    benchLen: benchData.length,
    fusedLen: fusedData.length,
    bench0: benchData[0],
    fused0: fusedData[0],
    sameArray: benchData === fusedData
  });
  ...
  // --- SMYČKA ---
  function tick(){
    if (!bothActive || bothIdx >= fusedData.length) { bothActive = false; return; }

    ensureBothMarkers();

    const f = fusedData[bothIdx];
    const b = nearestByTime(benchData, f.time, 12);   // ±12 s okno

    // rychlosti
    const f_mps = Number.isFinite(f.speed_mps) ? f.speed_mps : (speedFromNeighbors(fusedData, bothIdx) ?? 0);
    let b_mps = 0, bIdx = -1;
    if (b) {
      bIdx  = benchData.indexOf(b);
      b_mps = Number.isFinite(b.speed_mps) ? b.speed_mps : (speedFromNeighbors(benchData, bIdx) ?? 0);
    }

    // metriky
    const d_m   = b ? haversine(f.lat, f.lng, b.lat, b.lng) : null;
    ...
    // texty do popupů
    const tFmt = { hour12:false, hour:'2-digit', minute:'2-digit', second:'2-digit', fractionalSecondDigits:3 };
    const fTimeStr = new Date(f.time).toLocaleTimeString('cs-CZ', tFmt);
    const bTimeStr = b ? new Date(b.time).toLocaleTimeString('cs-CZ', tFmt) : '—';
    const segF = segmentNear(f.lat, f.lng);
    const segB = b ? segmentNear(b.lat, b.lng) : '—';
    const fKmh = (f_mps*3.6).toFixed(1);
    const bKmh = (b_mps*3.6).toFixed(1);

    if (window.markerF?.getPopup) {
      let pf = window.markerF.getPopup();
      if (!pf) { window.markerF.bindPopup('', { closeButton:false, autoClose:false, closeOnClick:false, offset:[0,-10] }).openPopup(); pf = window.markerF.getPopup(); }
      const mline = (f.mesh_id != null)
        ? `<div><b>Shoda kotev:</b> ${f.matched_count}${(Array.isArray(f.matched_ids)&&f.matched_ids.length)?` ([${f.matched_ids.join(', ')}])`:''}</div>`
        : `<div><b>Shoda kotev:</b> 0</div>`;
      pf.setContent(
        `<div style="font-size:11px">
           <b>F_GPS</b><br>
           <b>TIME:</b> ${fTimeStr}<br>
    ...
    if (window.markerB?.getPopup) {
      let pb = window.markerB.getPopup();
      if (!pb) { window.markerB.bindPopup('', { closeButton:false, autoClose:false, closeOnClick:false, offset:[0,-10] }).openPopup(); pb = window.markerB.getPopup(); }
      pb.setContent(
        `<div style="font-size:11px">
           <b>GNSS</b><br>
           <b>TIME:</b> ${bTimeStr}<br>
           <b>GPS:</b> ${b ? `${b.lat.toFixed(6)}, ${b.lng.toFixed(6)}` : '—'}<br>
           <b>SEGMENT:</b> ${segB}<br>
           <b>Rychlost:</b> ${b ? bKmh : '—'} km/h
         </div>`
      );
    }
    ...
    if (info) {
      ...
      const zoneHtmlB = b ? zoneBadgeFor(b.lat, b.lng) : zoneHtmlF;
      info.innerHTML = `
        <b>Porovnání (F_GPS vs GNSS)</b>
        <hr style="margin:5px 0">
        <div>${zoneHtmlF} &nbsp;|&nbsp; ${zoneHtmlB}</div>
        <b>ΔTIME:</b> ${lag_s != null ? lag_s.toFixed(2)+' s' : '—'} &nbsp;·&nbsp;
        <b>ΔDIST:</b> ${d_m != null ? d_m.toFixed(2)+' m' : '—'} &nbsp;·&nbsp;
    ...
    // bezpečný debug uvnitř scope
    // console.log('TICK', bothIdx, { fT: fTimeStr, bT: bTimeStr, d_m, lag_s });

    // další krok
    const nextMs = (bothIdx < fusedData.length - 1) ? fusedData[bothIdx + 1].time : (f.time + 1000);
    const delay  = Math.max(10, (nextMs - f.time) / (playbackSpeed || 1));
    bothIdx++;
    bothTimer = setTimeout(tick, delay);
  }

  // první krok
  bothTimer = setTimeout(tick, 0);
}


  console.log('DBG sources:',
    { fused0: fusedData[0], bench0: benchData[0], sameArray: fusedData === benchData }
  );
  console.log('TICK', bothIdx, {
    fT: new Date(f.time).toISOString(),
    bT: b ? new Date(b.time).toISOString() : null,
    fLL: [f.lat, f.lng],
    bLL: b ? [b.lat, b.lng] : null,
    d_m
  });
