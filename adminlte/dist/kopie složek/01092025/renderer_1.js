...
function updateFgpsHud(f){
  if (!f) return;
  const i   = Math.max(1, fIdx);
  const mps = Number.isFinite(f.speed_mps) ? f.speed_mps : (speedFromNeighbors(fusedData, i) ?? 0);
  const kmh = mps * 3.6;
  const t   = fmtTime(msOf(f.time));
  const seg = segmentNear(f.lat, f.lng);
  const ids = anchorsNearest(msOf(f.time)); // BASIC_TABLE – nejbližší kotvy ±5s
  document.getElementById('fgps-hud').innerHTML = `
    <b>F_GPS</b><br>
    <b>Čas:</b> ${t}<br>
    <b>GPS:</b> ${f.lat.toFixed(6)}, ${f.lng.toFixed(6)}<br>
    <b>Segment:</b> ${seg}<br>
    <b>Rychlost:</b> ${kmh.toFixed(1)} km/h<br>
...
function updateGnssHud(b){
  if (!b) return;
  const i   = Math.max(1, bIdx);
  const mps = Number.isFinite(b.speed_mps) ? b.speed_mps : (speedFromNeighbors(benchData, i) ?? 0);
  const kmh = mps * 3.6;
  const t   = fmtTime(msOf(b.time));
  const seg = segmentNear(b.lat, b.lng);
  const ids = meshAnchorsNearest(b.lat, b.lng); // MESH footprinty
...
if (ballInfo) {
  ...
  const tStr = fmtTime(msOf(rec.time));   // místo rec.time.toLocaleTimeString()
...
let FGPS_LAG_MS = 10000; // klidně dej 5000–10000 dle potřeby
...
// BASIC_TABLE – kotvy nejbližší k času (±5s, preferuj menší |dt|)
function anchorsNearest(ms, maxWindowSec=5) {
  let best = [], bestAbs = Infinity;
  for (let dt=0; dt<=maxWindowSec; dt++){
    for (const sign of (dt===0 ? [1] : [1,-1])) {
      const t = ms + sign*dt*1000;
      const d = new Date(t);
      const hh = String(d.getUTCHours()).padStart(2,'0');
      const mm = String(d.getUTCMinutes()).padStart(2,'0');
      const ss = String(d.getUTCSeconds()).padStart(2,'0');
      const ids = anchorsAtTime(`${hh}:${mm}:${ss}`) || [];
      if (ids.length && Math.abs(sign*dt) < bestAbs) { best = ids.slice(); bestAbs = Math.abs(sign*dt); }
    }
...
function maybeSnapFgps(i){
  ...
  // BASIC okno dopředu 28 s, max 6 řádků
  const counts = anchorsWindowForward(fMs, 28, 6);
  if (!counts || counts.size === 0) return;

  const scores = segmentScoresFromAnchors(counts);
  let seg = chooseSegment(scores);
  if (!seg) return;

  // hysteréze: min. doba po přepnutí
  const FREEZE_MS = 7000;
  if (fgpsSegState.current && seg !== fgpsSegState.current) {
    if (fMs - (fgpsSegState.lastSwitchMs || 0) < FREEZE_MS) {
      seg = fgpsSegState.current;
    }
  }
  ...
  // křižovatka (37/38/45) → větší snap rádius
  const nearJunction = counts.has(37) || counts.has(38) || counts.has(45);
  const SNAP_R = nearJunction ? 35 : 15;

  const snap = nearestMeshInSegment(seg, f.lat, f.lng);
  if (snap && snap.dist <= SNAP_R) {
...
function anchorsWindowForward(tMs, aheadSec = 25, maxRows = 6){
  if (!Array.isArray(BASIC) || !BASIC.length) return new Map();

  const t0 = secOfDayUTC(tMs);
  const rows = BASIC.filter(r=>{
    const s = parseBasicTimeToSec(r.TIME || r.time || r.Timestamp || r.timestamp);
    return s!=null && s >= t0 && s <= t0 + aheadSec;
  }).slice(0, maxRows);

  const counts = new Map(); // kotva -> počet výskytů
  rows.forEach(r=>{
...
function updateBothInfoPanel(fIndex, bIndex) {
  ...
  const lag_s = (msOf(f.time) - msOf(b.time)) / 1000; // žádné přičítání konstanty
  ...
  const fTimeStr = fmtTime(msOf(f.time));
  const bTimeStr = fmtTime(msOf(b.time));
