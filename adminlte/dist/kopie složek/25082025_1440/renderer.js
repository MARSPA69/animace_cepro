...
function makeAnimSeries(src) {
  return (src || [])
    ...
    .map(d => ({
      ...
      // v SINGLE režimu chceme Date (kvůli .toLocaleTimeString)
      time:  (typeof d.time==='number') ? new Date(d.time)
...
function applyChannel() {
  ...
  if (mode === 'both') {
  // vypnout single animaci a odstranit černou kuličku
    stopSingle();

  // připrav benchData z GNSS, pokud ještě není
  if (!benchData.length && Array.isArray(window.realData) && window.realData.length) {
    ...
    // vytvoř/posuň jen barevné kuličky na start; RUN spustí bothRun()
    bothResetState();
    bothSetStartPositions();
    return;
  } else {
  // opouštím BOTH → uklidit barevné kuličky, aby zůstal jen single
    stopBoth();
  }
}
...
function bothRun(){
  ...
  function tick(){
    ...
    const b = nearestByTime(benchData, f.time, 12);

    const f_mps = (typeof f.speed_mps === 'number') ? f.speed_mps : (speedFromNeighbors(fusedData, bothIdx) ?? 0);
    let b_mps = 0;
    if (b) {
      const j = benchData.indexOf(b);
      b_mps = (typeof b.speed_mps === 'number') ? b.speed_mps : (speedFromNeighbors(benchData, j) ?? 0);
    }
    ...
    // panel – jen to podstatné: Δdist, Δtime, Δspeed + MATCH F_GPS
    const ballInfo = document.getElementById('ball-info-content');
    if (ballInfo) {
      const matchLine = (f.mesh_id != null)
        ? `<b>MATCH:</b> ${f.matched_count}${
            (Array.isArray(f.matched_ids) && f.matched_ids.length) ? ` ([${f.matched_ids.join(', ')}])` : ''
          } · <b>MESH:</b> ${f.mesh_id}<br>`
        : `<b>MATCH:</b> 0<br>`;
      ballInfo.innerHTML = `
        <b>Porovnání (obě)</b><hr style="margin:5px 0">
        <b>Čas (F_GPS):</b> ${new Date(f.time).toLocaleTimeString()}<br>
        <b>ΔDIST:</b> ${d_m != null ? d_m.toFixed(2)+' m' : '—'}
         · <b>ΔSPEED:</b> ${d_spd.toFixed(2)} m/s
         · <b>ΔTIME:</b> ${lag_s != null ? lag_s.toFixed(2)+' s' : '—'}<br>
        <b>F_GPS:</b> ${f.lat.toFixed(6)}, ${f.lng.toFixed(6)} · ${(f_mps*3.6).toFixed(1)} km/h<br>
        <b>GNSS:</b> ${b ? `${b.lat.toFixed(6)}, ${b.lng.toFixed(6)} · ${(b_mps*3.6).toFixed(1)} km/h` : '—'}<br>
        ${matchLine}
      `;
    }
    ...
    window.bothPopup
      ...
      .setContent(
        `<div style="font-size:11px">
           Δdist: ${d_m != null ? d_m.toFixed(2)+' m' : '—'}
           · Δspeed: ${d_spd.toFixed(2)} m/s
           · Δtime: ${lag_s != null ? lag_s.toFixed(2)+' s' : '—'}<br>
