...
function downloadFusedLog(filename) {
  try {
    ...
    // Připravíme payload ve formátu, který chceš:
    // TIME (z BASIC_TABLE), F_GPS, matched IDs (pokud nastal hit)
    ...
    // Název souboru F_GPS_YYYY-MM-DD.js (pokud nepředán)
    const defName = `F_GPS_${new Date().toISOString().slice(0,10)}.js`;
    const outName = filename || defName;

    const js = `// AUTO-GENERATED\nwindow.F_GPS_LOG = ${JSON.stringify(items, null, 2)};\n`;
