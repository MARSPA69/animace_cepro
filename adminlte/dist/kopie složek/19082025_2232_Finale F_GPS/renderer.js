...
animationData = (window.realData || [])
  ...
  .map(d => {
    ...
    // přenes extra pole z FUSED_GPS (pokud existují)
    if ('speed_mps' in d)     rec.speed_mps     = d.speed_mps;
    if ('dist_to_m' in d)     rec.dist_to_m     = d.dist_to_m;
    if ('mesh_id' in d)       rec.mesh_id       = d.mesh_id;
    if ('matched_count' in d) rec.matched_count = d.matched_count;
    if ('matched_ids' in d)   rec.matched_ids   = d.matched_ids;
    return rec;
  });
}    
...
function getNearbyMesh(lat, lon) {
  if (!window.meshMeshMarkers) return null;

  const LIMIT = window.meshMaxDist ?? 5;   // ← hodnota z inputu (fallback 5 m)
  let nearest = null;
  let minDist = LIMIT + 1;                 // hledáme jen body ≤ LIMIT

  window.meshMeshMarkers.forEach(m => {
    const d = window.leafletMap.distance(
                L.latLng(lat, lon), m.getLatLng());   // metry
    if (d <= LIMIT && d < minDist) {
      minDist = d;
      nearest = { marker: m, dist: d, data: m.data };
    }
  });

  return nearest;     // null, když nic nesplní LIMIT
}
...
// zvýraznění + popup MESH hit 
const meshExtra = document.getElementById('mesh-extra');

if (meshInfo) {
  /* 1) zruš staré zvýraznění, je-li jiné */
  ...
  if (meshExtra){
    meshExtra.innerHTML =
      `<b>MESH hit ▶</b> Footprints: ${meshInfo.data.Footprints.join(', ')} |
       Segment: ${meshInfo.data.Segment}`;
  }

  /* 4) Leaflet-popup nad kuličkou */
  if (!window.meshPopup) {
    window.meshPopup = L.popup({ offset:[0,-10], closeButton:false });
  }
  window.meshPopup
    .setLatLng([rec.lat, rec.lng])               // souřadnice kuličky
    .setContent(
      `<div style="font-size:11px">
         <b>Footprint:</b> ${meshInfo.data.Footprints.join(', ')}<br>
         <b>Segment:</b> ${meshInfo.data.Segment}
       </div>`
    )
    .openOn(window.leafletMap);

} else {        // — žádný hit: vše vrátíme —
  if (prevMeshMarker){
    prevMeshMarker.setStyle({ color:'#28a745', fillColor:'#28a745' });
    prevMeshMarker = null;
  }
...
if (ballInfo) {
  const matchHtml = (mode === 'offlinegnss' && rec.mesh_id != null)
    ? `<b>MATCHED:</b> ${rec.matched_count} ${rec.matched_count ? `([${rec.matched_ids.join(', ')}])` : ''}<br><b>MESH ID:</b> ${rec.mesh_id}<br>`
    : '';

  const timeText = (rec.time instanceof Date) ? rec.time.toLocaleTimeString() : (rec.timestamp || '');
  ...
  ballInfo.innerHTML = `
    ...
    ${matchHtml}
    <b>ID:</b> ${typeof SUBJECT_ID !== 'undefined' ? SUBJECT_ID : '—'}
  `;
}
...
if (mode === 'offlinegnss') {
  ...
  const mpsLbl = mps.toFixed(2); // už je spočítané výše
  ...
  window.fgpsPopup
    ...
    .setContent(
      `<div style="font-size:11px">
        ...
        v: ${mps.toFixed(2)} m/s · d→MESH: ${distTxt}
        ${matchLine}
      </div>`
    )
    .openOn(window.leafletMap);
...
function applyChannel() {
  ...
  // Mesh overlay chceme pro MESH i OFFLINE i BOTH
  ...
  // OFFLINE GNSS: spočítej syntetiku a spusť animaci
  if (mode === 'offlinegnss' && window.FUSED_GPS) {
    // propsat limit z inputu do FUSED_GPS
    if (typeof window.meshMaxDist === 'number' && window.FUSED_GPS.setSnapDistance) {
      window.FUSED_GPS.setSnapDistance(window.meshMaxDist);
    }
    window.FUSED_GPS.runOfflineGNSS();  // to vyrobí fused a zavolá applyFusedGpsDataset()
  }
  // Pozn.: "both" necháme na další krok (druhá kulička).
}
...
document.addEventListener('DOMContentLoaded', () => {
  const btn = document.getElementById('btn-offline-gnss');
  if (btn) {
    btn.addEventListener('click', () => {
      if (window.FUSED_GPS) window.FUSED_GPS.runOfflineGNSS();
    });
  }
  const btnSave = document.getElementById('btn-save-fgps');
  if (btnSave) {
    btnSave.addEventListener('click', () => {
      if (!window.FUSED_GPS) return;
...
document.getElementById('btn-save-fused-log')?.addEventListener('click', () => {
  // navrhneme jméno: F_GPS_YYYY-MM-DD.js
  const fname = `F_GPS_${new Date().toISOString().slice(0,10)}.js`;
  window.FUSED_GPS?.downloadFusedLog?.(fname);
});
...
/* === HOOK PRO OFFLINE GNSS (vložit NA KONEC renderer.js, za DOMContentLoaded) ===
   Pozn.: startAnimation() u tebe čeká window.realData. Proto mu ji tady naplníme
   z fused-výstupu a použijeme standardní startAnimation bez dalších zásahů. */
...
window.applyFusedGpsDataset = function (fused) {
  ...
  // Překlop do tvaru očekávaného startAnimation()
  window.realData = fused.map(r => ({
    lat: r.lat,
    lng: r.lng,
    // renderer počítá s Date z ISO – necháme den fiktivní:
