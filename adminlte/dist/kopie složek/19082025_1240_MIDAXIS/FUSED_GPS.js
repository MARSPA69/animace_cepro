...
function buildFusedSeries() {
  ...
  const TABLE = Array.isArray(TABLESRC) ? TABLESRC : [];

  const FOOT_SRC = getFootSrc();

  console.log("FUSED inputs:",
    "MGPS", MGPS.length,
    "TABLE", TABLE.length,
    "FOOT_SRC", Array.isArray(FOOT_SRC) ? `array(${FOOT_SRC.length})` : typeof FOOT_SRC
  );
  if (!MGPS.length || !TABLE.length) {
    console.warn("FUSED_GPS: chybí FIXED_GPS_MESH nebo BASIC_TABLE_04062025.");
    return [];
  }

  // Trasa vpřed (greedy) a index dalšího cíle
  const PATH = orderMeshPath(MGPS, CFG.START.lat, CFG.START.lng);
  let pathIdx = 0; // cílem je vždy PATH[pathIdx + 1]

  // Přemapuj řádky a deduplikuj čas
  const rows = dedupRows(TABLE).map(r => {
    const ts  = getRowTimestamp(r);
    const sec = parseHmsToSec(ts);
    return { raw: r, ts, sec, speed: getRowSpeed(r), a_ids: readAnchors(r) };
  }).filter(x => x.sec != null);

  // Index pro volitelný kompas
  const rowsBySec = new Map();
  for (const rr of rows) {
    if (!rowsBySec.has(rr.sec)) rowsBySec.set(rr.sec, []);
    rowsBySec.get(rr.sec).push(rr.raw);
  }

  // Start stav
  let fusedLat = CFG.START.lat;
  let fusedLng = CFG.START.lng;
  let prevSec  = parseHmsToSec(CFG.START.ts);

  const out = [];

  for (let i = 0; i < rows.length; i++) {
    const { ts, sec, speed, a_ids } = rows[i];
    const dt = (prevSec == null) ? 0 : Math.max(0, sec - prevSec);

    // ► CÍL: „další“ uzel v trase (monotónně vpřed)
    const nextNode = PATH[Math.min(pathIdx + 1, PATH.length - 1)];

    // ► POSUN: k „dalšímu“ uzlu
    if (speed > 0 && dt > 0 && nextNode) {
      const dToNext = haversine_m(fusedLat, fusedLng, nextNode.lat, nextNode.lng);
      const stepM   = speed * dt;

      if (stepM >= dToNext) {
        // dorazíme/přestřelíme → sedni přesně na nextNode
        fusedLat = nextNode.lat;
        fusedLng = nextNode.lng;
      } else {
        const br  = bearing_deg(fusedLat, fusedLng, nextNode.lat, nextNode.lng);
        const dst = destinationPoint(fusedLat, fusedLng, br, stepM);
        fusedLat = dst.lat;
        fusedLng = dst.lng;
      }
    }

    // ► SNAP: pouze na nextNode (ne na „libovolný nejbližší“)
    let snapped = false, matchPct = 0, anchorFoot = null, m_id = null;
    let dNext = 0;
    if (nextNode) {
      dNext = haversine_m(fusedLat, fusedLng, nextNode.lat, nextNode.lng);
      const fpNext = footprintForId(nextNode.id, FOOT_SRC);
      matchPct = matchPercent(a_ids, fpNext);

      // tvrdý snap ≤ FORCE_SNAP_M, jinak měkký v dosahu se shodou kotev
      if (dNext <= (CFG.FORCE_SNAP_M || 1.0) ||
         (dNext <= CFG.SNAP_DISTANCE_M && matchPct >= CFG.MATCH_THRESHOLD)) {
        fusedLat = nextNode.lat;
        fusedLng = nextNode.lng;
        snapped  = true;
        m_id     = nextNode.id;
        anchorFoot = uniq(fpNext.map(Number).filter(Number.isFinite));
        if (pathIdx < PATH.length - 1) pathIdx++; // posuň VPŘED
      }
    }

    // (jen pro informaci v panelu) nejbližší libovolný MESH
    const nearB = nearestMGPS(fusedLat, fusedLng, MGPS);

    const compass = inferCompassAt(sec, rowsBySec);
    ...
    out.push({
      ...
      lat: fusedLat,
      lng: fusedLng,
      // můžeš si zvolit: buď vzdál. k nextNode (dNext), nebo k nejbližšímu MESH (nearB.dist)
      dist_to_m: Math.round((dNext || nearB.dist) * 1000) / 1000,
      snapped,
      m_id: snapped ? m_id : null,
      match_pct: Math.round(matchPct * 1000) / 1000,
      anchor_footprint: anchorFoot,
      compass: compass ? {
        code: compass.code,
        compass_deg: compass.compass_deg,
        min_deg: compass.min_deg,
        max_deg: compass.max_deg,
        direction: compass.direction,
        sequenceKey: compass.sequenceKey
      } : null
    });
  ...
  window.FUSED_GPS = {
    setSnapDistance(m) {
      const v = Number(m);
      if (Number.isFinite(v) && v > 0) CFG.SNAP_DISTANCE_M = v;
    },
    buildFusedSeries,
    runOfflineGNSS,
    downloadFgpsJs,
