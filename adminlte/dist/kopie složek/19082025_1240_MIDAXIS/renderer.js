...
/* … předchozí kód DOMContentLoaded … */
...
window.applyFusedGpsDataset = function (fused) {
  ...
  window.realData = fused.map(r => {
    const t = String(r.timestamp || "00:00:00").trim();
    const [hh,mm,ss] = t.split(":").map(x => String(x).padStart(2,"0"));
    return { lat: r.lat, lng: r.lng, timestamp: `1970-01-01T${hh}:${mm}:${ss}Z` };
  });
