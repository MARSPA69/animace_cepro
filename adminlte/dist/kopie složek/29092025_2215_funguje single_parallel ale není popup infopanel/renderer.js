...
function enrichRecordForParallel(rec, prevRec) {
  ...
  if (!rec.timeStr) {
    try {
      ...
      if (typeof rec.timestamp === "string") {
        ...
        // Firestore-like objekt {seconds, nanoseconds}
  ...
  if (!rec.point) {
    try {
      ...
      console.warn("❌ rec bez validních souřadnic:", rec);
    }
  }

  // 3) Rychlost
  if (typeof rec.speed_mps !== "number" && prevRec && prevRec.point) {
    try {
      const distKm = turf.distance(prevRec.point, rec.point, { units: "kilometers" });
      const dt = (rec.time - prevRec.time) / 1000;
...
if (mode === 'offlinegnss') {
  ...
  processRecord(rec, { marker: window.marker, panel: ballInfoPanel, dataset: animationData, idx });       // místo přímého updateMarkerPopup()
...
function parallelStep() {
  ...
  window.parallelInstances.forEach(track => {
    ...
    enrichRecordForParallel(rec, prev);   // 🔧 obohacení dat

    processRecord(rec, track);            // 🔧 update panelů & markerů
