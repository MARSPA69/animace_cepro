...
ballInfoPanel = createBallInfoPanel();
...
function createBallInfoPanel() {
  if (document.getElementById('ballInfoPanel')) {
    return document.getElementById('ballInfoPanel');
  }
  const panel = document.createElement('div');
...
function processRecord(rec, track) {
  ...
  // 2) Update panel (Čas, GPS, Rychlost, Stav)
...
function startAnimation() {
  ...
  // pokud jsme v single módu, vytvoř panel
...
function startParallelAnimation() {
  ...
  panelContainer.innerHTML = ""; // vyčisti staré boxy
  ...
  for (const [id, track] of Object.entries(window.parallelTracks)) {
    ...
    // marker na mapě
    track.marker = L.circleMarker([track.dataset[0].lat, track.dataset[0].lng], {
      radius: 6,
      color: track.color,
    ...
    const panel = createParallelPanel(id, track.color);
    document.getElementById("parallel-panels").appendChild(panel);
    track.panel = panel;

    // malý infobox vlevo dole
    panel.className = "parallel-panel";
    panel.style.background = "rgba(255,255,255,0.95)";
    panel.style.color = "#000";
    panel.style.padding = "6px 10px";
    ...
    panel.style.maxWidth = "220px";
    panel.style.boxShadow = "0 2px 6px rgba(0,0,0,0.15)";
    panel.style.borderLeft = `6px solid ${track.color}`;

    panel.innerHTML = `
      <div><b style="color:${track.color}">${id}</b></div>
      <div class="info-item"><span class="info-label">Čas:</span> <span class="info-value">—</span></div>
      <div class="info-item"><span class="info-label">GPS:</span> <span class="info-value">—</span></div>
      <div class="info-item"><span class="info-label">Rychlost:</span> <span class="info-value">—</span></div>
      <div class="info-item"><span class="info-label">Stav:</span> <span class="info-value">—</span></div>
