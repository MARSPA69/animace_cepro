...
// --- Mapa a marker ---
  const map = L.map('leafletMap').setView([greenCenter[1], greenCenter[0]], 17);
...
// polygonové vrstvy – pouze ty, bez kotev a bez CSS
  ...
  L.geoJSON(bigPoly,       { color:'#dc3545', weight:3, dashArray:'5,10', fillOpacity:0 }).bindPopup('Zakázaná zóna').addTo(map);
    ...
    console.log('>>> renderer.js – před AF.init, map =', map);
...
const step = () => {
  ...
  if (idx > 0) {
    ...
    // <<< AF-TICK: kotvová kulička
if (anchorActive && window.AF) {
  const res = window.AF.tick(delay);         // delay = spočtený ms mezi body
  if (!window.ballAnchor) {
    window.ballAnchor = L.circleMarker([res.lat, res.lon], {
      radius: 7, color: "#ff6600", fillColor: "#ffaa00", fillOpacity: 0.9
    }).addTo(map);
  } else {
    window.ballAnchor.setLatLng([res.lat, res.lon]);
  }
  if (res.matched) console.log("★ FIXED GPS:", res.label);
} else if (window.ballAnchor) {
  window.ballAnchor.setStyle({ opacity: 0, fillOpacity: 0 }); // skryj, není-li aktivní
}
...
function applyChannel() {
  ...
  const mode       = channelSel.value;               // none | gnss | mesh | both

  const gnssOn     = (mode === "gnss"  || mode === "both");
  const meshOn     = (mode === "mesh" || mode === "both");

  // –– Fixed-GPS mesh (zelené body)
  if (window.AF && typeof window.AF.init === "function") {
    window.AF.init(map);       // ← this sets the IIFE’s internal `map` variable
    window.AF.showMesh(false); // ← now it can safely add/remove layers
  }

  // –– GNSS animace
  animationActive = gnssOn;
  if (!animationActive && window.timer) {
    clearTimeout(window.timer);
    window.timer = null;
  }
  if (gnssOn && !window.timer) {
    startAnimation();
  }
}
...
document.getElementById('startBtn')?.addEventListener('click', () => {
  ...
  if (!animationActive) {
    ...
    playbackSpeed = 1;
    if (!window.timer && typeof window.animationStep === "function") {
