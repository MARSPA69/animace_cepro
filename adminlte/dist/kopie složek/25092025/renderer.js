...
function updateMarkerPopup(rec, calculatedSpeed = null, inGreen = null, inRed = null) {
  ...
  const popupContent = `
    ...
    <b>Zóna:</b> <span style="color: ${zoneStatus === 'GREEN' ? 'green' : zoneStatus === 'RED' ? 'red' : 'gray'}">${zoneStatus}</span><br>
    <b>Rychlost:</b> ${speed.toFixed(2)} m/s (${kmh.toFixed(1)} km/h)<br>
    <b>Typ pohybu:</b> <span style="color: ${getMovementTypeColor(speed)}">${movementType}</span>
  `;
