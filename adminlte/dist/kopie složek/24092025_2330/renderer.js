...
function getMovementType(speed) {
  if (speed < 0.2) return "Stojí";
  if (speed < 1.5) return "Chůze";
  return "Běh";
}

function updateMarkerPopup(rec) {
  if (!window.marker || !rec) return;

  const zoneStatus = checkZoneStatus(rec); // "GREEN" nebo "RED"
  const speed = rec.speed_mps || 0;
  const movementType = getMovementType(speed);

  const popupContent = `
    <b>Čas:</b> ${rec.timeStr}<br>
    <b>GPS:</b> ${rec.lat.toFixed(6)}, ${rec.lng.toFixed(6)}<br>
    <b>Zóna:</b> <span style="color: ${zoneStatus === 'GREEN' ? 'green' : 'red'}">${zoneStatus}</span><br>
    <b>Rychlost:</b> ${speed.toFixed(2)} m/s<br>
    <b>Typ pohybu:</b> ${movementType}
  `;
...
function startAnimation() {
  ...
  const currentGnssDataset =
    window.currentGnssDataset ||   // 👈 preferujeme dataset vybraný v dropdownu
    getCurrentGnssDataset();       // fallback na původní logiku
  ...
  console.log("animationData length (before):", animationData.length);
  console.log("idx:", idx);
  ...
  // 🟢 2. Zjištění aktuálního režimu z UI
  ...
  // 🟢 7. Vytvoření nebo update markeru
  ...
  if (!window.marker) {
    ...
    .bindPopup("Načítám data...")   // 👈 základní popup
    .addTo(map);
  } else {
    const firstDataPoint = animationData[0];
    window.marker.setLatLng([firstDataPoint.lat, firstDataPoint.lng]);
    window.marker.setPopupContent("Načítám data...");  // 👈 reset obsahu
  }

  // 🟢 8. Inicializace stavu animace
  animationActive = true;
  playbackSpeed = 1;
  updateSpeedDisplay();
  idx = 0;

  // 🟢 9. Reset timeru a spuštění smyčky
  if (window.timer) {
...
function bothRun() {
  ...
  const src = (Array.isArray(currentGnssDataset) && currentGnssDataset.length)
    ? makeAnimSeries(currentGnssDataset)
