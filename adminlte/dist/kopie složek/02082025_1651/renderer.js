...
const step = () => {
  ...
  if (meshInfo && meshExtra) {
    meshExtra.innerHTML =
      `<b>MESH hit ▶</b> Footprints: ${meshInfo.data.Footprints.join(', ')} |
      Segment: ${meshInfo.data.Segment}`;
  } else if (meshExtra) {
  meshExtra.innerHTML = '';      // odjedeme-li pryč, řádek zmizí
  } 

  if (meshInfo) {
    // zobrazíme jednoduchý popup u kuličky
    if (!window.meshPopup) {
      window.meshPopup = L.popup({ offset: [0, -10], closeButton: false });
    }
    const fp = meshInfo.data.Footprints.join(', ');
    window.meshPopup
