...
function initParallelUI() {
  ...
  if (panel && !panel._bound) {
    panel.addEventListener('click', (e) => {
      ...
      const ds = btn.dataset.dataset;  // např. RENDERERDATA1
      ...
      if (btn.classList.contains('active')) {
        ...
        // Nastav barvu tlačítka podle datasetu
...
function createBallInfoPanel() {
  // Pokud už panel existuje, vrať ho
  let panel = document.getElementById('ballInfoPanel');
...
function enrichRecordForParallel(rec, prevRec) {
  if (!rec) return rec;

  // Časový string
  if (!rec.timeStr) {
    if (rec.timestamp) {
      rec.timeStr = parseTimeString(rec.timestamp); // stejné jako v step()
    } else {
      rec.timeStr = "—";
    }
  }
  ...
  // Výpočet rychlosti
  if (!rec.speed_mps && prevRec) {
    try {
      const distKm = turf.distance(
        turf.point([prevRec.lng, prevRec.lat]),
        turf.point([rec.lng, rec.lat]),
        { units: "kilometers" }
      );
      const dt = (new Date(rec.timestamp) - new Date(prevRec.timestamp)) / 1000;
      if (dt > 0) {
        rec.speed_mps = (distKm * 1000) / dt;
      } else {
        rec.speed_mps = 0;
      }
    } catch (e) {
  ...
  if (!rec.zoneStatus) {
    if (checkZoneStatus) {
      rec.zoneStatus = checkZoneStatus(rec);
    } else {
      // fallback na inGreen/inRed
...
function processRecord(rec, track) {
  ...
  if (rows && rows.length >= 4) {
    ...
    rows[2].textContent = (typeof rec.speed_mps === 'number' ? rec.speed_mps.toFixed(2) : "0.00") + " m/s";
...
if (mode === 'offlinegnss') {
  ...
  processRecord(rec);                                 // místo přímého updateMarkerPopup()
...
function startParallelAnimation() {
  ...
  for (const [id, track] of Object.entries(window.parallelTracks)) {
    ...
    track.marker = L.circleMarker(                                                                     // marker
      [track.dataset[0].lat, track.dataset[0].lng],
      { radius: 6, color: track.color, fillColor: track.color, fillOpacity: 0.9 }
    ).addTo(window.leafletMap);

    const panel = createParallelPanel(id, track.color);                                               // panel – použij JEN createParallelPanel
    document.getElementById("parallel-panels")?.appendChild(panel);
    track.panel = panel;

    track.idx = 0;                                                                                   // start index
...
function parallelStep() {
  ...
  window.parallelInstances.forEach(track => {
    ...
    // enrich záznam (čas, rychlost, zóna)
    const prev = track.idx > 0 ? track.dataset[track.idx - 1] : null;
    enrichRecordForParallel(rec, prev);

    processRecord(rec, track);

    track.idx++;
    active = true;
  });

  if (active && window.animationActive) {
...
function resumeAnimation() {
  ...
  if (window.parallelMode) {
    parallelStep();   // ✅ pokračuj paralelní smyčkou
  } else {
    step();           // ✅ pokračuj single smyčkou
  }
}
...
function stopAnimation() {
  ...
  idx = 0;
  for (const track of window.parallelInstances) {
    track.idx = 0;  // resetuj i paralelní datasety
  }
}
...
document.getElementById('startBtn')?.addEventListener('click', () => {
  ...
  if (mode === 'both') {
    ...
    if (window.parallelMode) {                                                              // SINGLE nebo PARALLEL
      if (window.parallelInstances.length === 0) {
    startParallelAnimation();                                                                      // ← NOVÁ FUNKCE pro parallel mode
    } else {
                                                                                      
      if (Array.isArray(animationData) && animationData.length && idx < animationData.length && !animationActive) {            // SINGLE – resume z místa
        animationActive = true;
        if (window.animTimer) { clearTimeout(window.animTimer); window.animTimer = null; }
        window.animTimer = setTimeout(step, 0);
      } else {
        resumeAnimation(); // první spuštění po loadu
      }
    }
  }
  }
...
document.getElementById('pauseBtn')?.addEventListener('click', () => {
  ...
  if (mode === 'both') {
    ...
    pauseAnimation();  // ← NOVÁ FUNKCE pro single/parallel mode
  }
});
...
document.getElementById('stopBtn')?.addEventListener('click', () => {
  ...
  if (mode === 'both') {
    ...
    stopAnimation();  // ← NOVÁ FUNKCE pro single/parallel mode
    
    if (window.parallelMode) {
      // Reset parallel markers to start positions
      window.parallelInstances.forEach(track => {
        if (track.marker && track.dataset[0]) {
          track.marker.setLatLng([track.dataset[0].lat, track.dataset[0].lng]);
        }
      });
    } else {
      // Single mode - reset marker to start
...
document.getElementById('fasterBtn')?.addEventListener('click', () => {
  ...
  if (inBoth) {
    ...
    scheduleNextF();
    tickGNSS();
  } else if (window.parallelMode) {
    // Parallel mode - speed change affects parallelStep()
    if (!animationActive && window.parallelInstances.length > 0) {
      animationActive = true; 
      parallelStep();
    }
  } else if (!animationActive && idx < animationData.length - 1 && !window.animTimer) {
