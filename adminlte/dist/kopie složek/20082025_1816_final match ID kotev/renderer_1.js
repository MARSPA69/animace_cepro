...
animationData = window.realData
  .filter(d =>
    ...
    (typeof d.timestamp === 'string' || typeof d.time === 'number')
  )
  .map(d => {
    const rec = {
      point: turf.point([d.lng, d.lat]),
      // preferuj numerický čas (ms) – je bez TZ posunů; jinak použij ISO string
      time: (typeof d.time === 'number') ? new Date(d.time) : new Date(d.timestamp),
      lat: d.lat,
      lng: d.lng
    };

    // přenes EXTRA pole z FUSED_GPS – přesně pod těmito názvy:
    if ('speed_mps'     in d) rec.speed_mps     = d.speed_mps;
    if ('dist_to_m'     in d) rec.dist_to_m     = d.dist_to_m;
    if ('mesh_id'       in d) rec.mesh_id       = d.mesh_id;
    if ('matched_count' in d) rec.matched_count = d.matched_count;
...
function getNearbyMesh(lat, lon) {
  if (!window.meshMarkers) return null;

  const LIMIT = window.meshMaxDist ?? 5;
  let nearest = null, minDist = LIMIT + 1;

  window.meshMarkers.forEach(m => {
    const d = window.leafletMap.distance(L.latLng(lat, lon), m.getLatLng());
