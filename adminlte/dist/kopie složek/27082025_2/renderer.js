...
let fIdx = 0;   // index ve fusedData
let bIdx = 0;   // index v benchData
let FGPS_LAG_MS = 10000; // F_GPS startovní zpoždění (10 s) 

// === GLOBÁLNÍ smyčka F_GPS ===
function scheduleNextF() {
  if (!bothActiveF || !Array.isArray(fusedData) || fIdx >= fusedData.length - 1) return;

  const cur  = fusedData[fIdx];
  const next = fusedData[fIdx + 1];

  const ct = (cur.time  instanceof Date) ? cur.time.getTime()  : Number(cur.time);
...
function tickGNSS() {
  if (!bothActiveB || !Array.isArray(benchData) || bIdx >= benchData.length - 1) return;
...
function startBothUiLoop(getFIdx, getBIdx) {
  stopBothUiLoop();
  uiTimer = setInterval(() => {
    const fi = getFIdx();
    const bi = getBIdx();

    if (fi === lastUi.fIdx && bi === lastUi.bIdx) return;
    lastUi = { fIdx: fi, bIdx: bi };

    const f = fusedData[ Math.min(fi, fusedData.length - 1) ];
    const b = benchData[ Math.min(bi, benchData.length - 1) ];
    if (!f || !b) return;

    // jen UI – žádná logika párování / žádné polygony v anim. smyčkách
    window.markerF?.setLatLng([f.lat, f.lng]);
    window.markerB?.setLatLng([b.lat, b.lng]);

    updateFgpsPopup(f);
    updateGnssPopup(b);
    updateBothInfoPanel(fi, bi);
  }, 100); // 10 Hz stačí, je to lehké a plynulé
}
...
// HUD stack (pravý dolní roh) – bude NAD tlačítky
// HUD stack (pravý dolní roh) – NAD tlačítky
const hudStack = document.createElement('div');
hudStack.id = 'hud-stack';
Object.assign(hudStack.style, {
  position: 'absolute',
  right: '10px',
  bottom: '110px',      // zvednuto nad "Export/Fullscreen" knoflíky
  zIndex: 1002,
  width: '260px',
  display: 'flex',
...
document.getElementById('map-wrapper')?.appendChild(hudStack);

// F_GPS HUD (nahoře)
const fgpsHud = document.createElement('div');
fgpsHud.id = 'fgps-hud';
Object.assign(fgpsHud.style, {
  background:'rgba(255,255,255,0.95)', border:'1px solid #ccc',
  borderRadius:'8px', padding:'8px', fontSize:'11px', minWidth:'240px'
});
hudStack.appendChild(fgpsHud);

// GNSS HUD (pod ním)
const gnssHud = document.createElement('div');
gnssHud.id = 'gnss-hud';
Object.assign(gnssHud.style, {
  background:'rgba(255,255,255,0.95)', border:'1px solid #ccc',
  borderRadius:'8px', padding:'8px', fontSize:'11px', minWidth:'240px'
});
hudStack.appendChild(gnssHud);

function updateFgpsHud(f){
  if (!f) return;
  const kmh = Number.isFinite(f.speed_mps) ? (f.speed_mps*3.6).toFixed(1) : '—';
...
function updateGnssHud(b){
  if (!b) return;
  const kmh = Number.isFinite(b.speed_mps) ? (b.speed_mps*3.6).toFixed(1) : '—';
...
document.getElementById('pauseBtn')?.addEventListener('click', () => {
  ...
  if (mode === 'both') {
    ...
    bothActiveB = false;
    if (bothTimerF) { clearTimeout(bothTimerF); bothTimerF = null; }
    if (bothTimerB) { cancelAnimationFrame(bothTimerB); bothTimerB = null; } // <— OPRAVA
    stopBothUiLoop(); // <— NOVÉ
  } else {
    animationActive = false;
    if (window.timer) { clearTimeout(window.timer); window.timer = null; }
...
function bothSetStartPositions(){
  ...
  // jen posun a otevřít existující popupy – žádné nové bindPopup tady
...
function bothRun() {
  ...
  // benchData: pokud existuje realData, zpracuj ji stejně jako v single
  ...
  benchData = src.map(r => ({
    lat: r.lat,
    lng: r.lng,
    time: (r.time instanceof Date) ? r.time.getTime() : Number(r.time),
    speed_mps: r.speed_mps ?? null
  })).sort((a,b)=>a.time-b.time);

  // F_GPS dataset připrav, když chybí
  if (!Array.isArray(fusedData) || !fusedData.length) {
    const raw = window.FUSED_GPS?.buildFusedSeries?.() || [];
    fusedData = raw.map(r => ({
      lat: r.lat, lng: r.lng,
  ...
  // Zarovnej pouze čas F_GPS na GNSS
  alignFusedToBenchDate();

   // fusedData = correctFgpsByAnchors(fusedData);
  // (Volitelně) Kotvové přiklepnutí F_GPS na správné segmenty:
  // pokud chceš hned teď, odkomentuj:
  // fusedData = correctFgpsByAnchors(fusedData);

  // Ukliď single i případný starý BOTH
  stopSingle();
  stopBoth();

  // Reset stavů a indexů
  bothActiveF = true;
  bothActiveB = true;
  fIdx = 0;
  bIdx = 0;

  // Markery – v režimu BOTH bez popupů
  ...
  // START: GNSS hned, F_GPS s počátečním zpožděním
  tickGNSS();
  bothTimerF = setTimeout(scheduleNextF, FGPS_LAG_MS);
}
...
function updateBothInfoPanel(fIdx, bIdx) {
  ...
  const lag_s = (f.time - b.time) / 1000;
...
function ensureBothMarkers_NoPopups() {
  // zavři/odvaž existující popupy (ghost fix)
